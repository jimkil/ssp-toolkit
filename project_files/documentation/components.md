...
Rather than grouping all controls into a single component file, these components are structured with a
...
This allows us to separate the controls into different files based on their families. Each control family is then defined
...
all the templates in the SSP Toolkit, the templates use
[Jinja2 templating syntax](https://jinja.palletsprojects.com/en/stable/) to allow for dynamic content generation based
on values stored in the `keys` directory in the project root. For instance, we store the project name in a key file
called `project.yaml`, which assigns the project name to the variable `project.name`.
